...
organizationalUnitName         = Template
commonName                     = Template CA
...
openssl req -x509 \
  ...
  -nodes -days 3650 \
  -subj "/O=Template,CN=Template CA" \
  -keyout "$DIR/ca.key" \
...
rm -rf $DIR/ca.pem $DIR/jwt.key $DIR/ca.key $DIR/*.srl $DIR/*.csr $DIR/*.cnf
